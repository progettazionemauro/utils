...
def apply_changes(root, xml_path, folder):
    """
    ...
    update_xml_content(root, changes)
...
def display_suggestions(suggestions, root, xml_path, folder):
    """
    Display suggestions in the Tkinter interface.
    """
    suggestion_text.config(state='normal')
    suggestion_text.delete(1.0, tk.END)
    
    if suggestions:
        suggestion_text.insert(tk.END, "Suggestions from AI for possible modifications:\n")
        for suggestion in suggestions:
            suggestion_text.insert(tk.END, f"{suggestion}\n")
    else:
        suggestion_text.insert(tk.END, "No highlighted text found in the document.")
...
suggestion_text = scrolledtext.ScrolledText(frame, wrap=tk.WORD, width=100, height=30)
suggestion_text.pack(pady=10, padx=10, fill=tk.BOTH, expand=True)
